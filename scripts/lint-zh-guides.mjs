...
const ABBREV = [
  { pattern: /送評/, label: '縮略：送評 → 提交鑑定' },
  { pattern: /(?<!查看現有|於網上查看批次現有)查進度/, label: '縮略：查進度 → 查看現有進度' },
  ...
  { pattern: /值得送|不宜送/, label: '縮略：值得送/不宜送 → 值得/不宜支付鑑定費用的情況包括…' },
  { pattern: /置中\s*\+\s*表面\s*\+\s*邊角/, label: '碎片：置中 + 表面 + 邊角 → 置中、表面狀況與邊角完整度' },
];
