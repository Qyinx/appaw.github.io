  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        '經驗法則：除非期望值（EV）達到現有卡磚價值的 1.5 至 2 倍，且您對現行 PSA 置中與表面標準有深刻理解，否則請保持原狀或僅申請換殼。香港藏家在破殼前，亦可親臨 [138 Arena](/business/psa-grading/) 門市，由 Appaw 現場初步評估置中比例、表面壓痕與邊角完整度，並預測可能的分數門檻；同時附設基本清潔保養，降低提交鑑定時因灰塵或油脂造成的扣分風險。',
      ],
      formula: {
        result: '期望值 (EV)',
        eyebrow: '破殼決策算式',
    ...
    },
    {
      ...
      paragraphs: [
        ...
        '這看似完美，但同樣面臨定錨效應與「隔殼觀火」的限制。評級員隔著厚重的 BGS 外殼很難精確判斷表面微小刮痕，因此在保守起見下，往往不會給出 10 分。因此，許多進階藏家在跨評時，仍會選擇先自行破殼，以獲取最公平的裸卡審視。若您已決定以裸卡重新提交，可透過 Appaw PSA 代送鑑定服務於 138 Arena 面交，由專人協助批次轉送並於網上查看現有進度。',
      ],
    },
  ],
  faq: [
    ...
    {
      q: '香港可以代送破殼重送或跨評嗎？',
      a: '可以。香港藏家可於銅鑼灣 138 Arena 面交，由 Appaw 提供現場卡況初步評估與基本清潔保養，再併入批次轉送 PSA。破殼前先做現場評估，有助判斷升分期望值是否值得承擔降級風險。',
    },
  ],
  midCta: {
    afterSectionId: 'expected-value-math',
    title: '破殼前，先做現場卡況初步評估',
    body: '香港藏家可親臨 138 Arena，由 Appaw 協助檢測置中、表面與邊角，並評估可能分數，再決定是否破殼重送。',
    primary: { label: 'PSA 代送鑑定', href: '/business/psa-grading/' },
    secondary: { label: '免費置中工具', href: '/tools/card-centering/' },
  },
  cta: {
    title: '重送前先評估，香港可面交代送',
    body: 'PSA 10 如今要求極度嚴格的正面 55/45 置中比例。決定破殼重送前，請先使用免費置中工具測量邊距；香港藏家亦可前往 138 Arena 面交，Appaw 提供專業驗卡與清潔保養，全力協助珍藏爭取最佳評級，並避免白白付出鑑定費用。',
    primary: { label: 'PSA 代送鑑定', href: '/business/psa-grading/' },
    secondary: { label: '免費置中工具', href: '/tools/card-centering/' },
  },
