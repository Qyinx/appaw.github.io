...
export const zh: Translations = {
  ...
  psaGradingPage: {
    ...
    advisor: {
      ...
      items: [
        ...
        {
          title: '挑選最聰明的提交鑑定方案',
          body: '當您的愛卡真有實力挑戰 PSA 10 時，我們會根據卡片市值與您的時間成本，為您在 Regular、Express 等不同方案中做出最划算的選擇。錢要花在刀口上，值得加急的我們才會建議升級。',
        },
      ],
    },
    trust: {
  ...
  centeringPage: {
    ...
    faq: [
      ...
      {
        q: '我手上的卡到底該「重新評級(Regrade)」還是「單純換新殼(Reholder)」？',
        a: '換新殼(Reholder)只會換掉刮花的塑膠殼，原本的 9 分還是 9 分。但重新評級(Regrade)等同於打掉重練，考官會重新檢查所有項目，有很高機率會降分！強烈建議先用我們的工具量一下：如果量出來置中數據達到 10 分水準，但現在標籤是 9 分，那就可以賭一把 Regrade；如果原本就偏了，那就乖乖花小錢換新殼就好。',
      },
      {
    ...
    tool: {
      ...
      guideModeEdge: '只看藍線(外緣)',
      guideModeBorder: '只看粉線(圖框)',
      guideModeBoth: '我全都要看',
      imageFilterLabel: '抓蟲濾鏡 (檢查瑕疵)',
      imageFilterOff: '原汁原味',
      imageFilterGrayscale: '黑白灰階',
      imageFilterContrast: '暴力高對比',
      imageFilterInvert: '負片反轉',
      imageFilterCycle: '切換濾鏡找瑕疵',
      imageFilterOffHint: '就是原本的照片，沒加料。',
      imageFilterGrayscaleHint: '切換成黑白。顏色干擾變少，用來抓刮痕或討厭的印刷線超好用。',
      imageFilterContrastHint: '把明暗對比開到最大。邊角的細微白邊或磨損瞬間現形。',
      imageFilterInvertHint: '變換成負片。白邊會變黑，專門對付難搞的水痕、污漬跟閃卡上的細微刮傷。',
      ...
      guideDismiss: '不再顯示教學',
      guideHandlesProgress: '幹得好！已定位 {done} / {total} 個控制點',
      ...
      photoModeRaw: '沒穿衣服的裸卡',
      photoModeSlab: '已經裝在殼裡的鑑定卡',
      verdicts: {
        regradeCandidate: {
          label: '強力推薦：去重新評級吧！',
          hint: '太狂了！這置中根本是 Gem Mint 等級。只要確定四角跟表面沒硬傷，大膽拿去拚高分吧！',
        },
        borderlineRegrade: {
          label: '有點危險：邊界案例',
        ...
        holdGrade: {
          label: '勸你冷靜：維持原狀就好',
          hint: '置中只有 NM–MT 水準，拿去重新評級大概率升不上去。如果只是嫌殼舊了，花點小錢「換新殼(Reholder)」就好。',
        },
        downgradeRisk: {
      ...
      reholderNote:
        '※ 溫馨提醒：「換新殼 (Reholder)」只是幫你換個乾淨的塑膠殼，原本的幾分就是幾分，不影響數字。我們的工具只能幫你量卡片有沒有印歪，沒辦法幫你看外殼是不是裂了。所以，這個鑑定卡模式主要用來判斷這張卡「值不值得敲碎重送拚高分」。',
    },
    content: {
      ...
      regradeAeoAnswer:
        '這是很多藏家的痛！簡單來說：「換新殼(Reholder)」很安全，就是花點錢把刮花的塑膠殼換掉，原本 9 分還是 9 分；但「重新評級(Regrade)」很刺激，考官會像看新卡一樣把四角跟置中重新抓出來挑毛病，沒過關直接降級！想玩刺激的？先用工具量置中：如果數據是完美的 Gem Mint，但現在標籤寫 9 分，那就有拚的價值；如果置中本來就爛，那勸你乖乖花錢換新殼就好。',
      regradeP1:
        '最嘔的就是：明明只是殼不小心摔裂了，結果選錯方案去「重新評級」，最後卡回來分數被降，真的是賠了夫人又折兵！',
      regradeP2:
        '你在網購平台或展會上看到的卡片照，十之八九都是手機隨便拍的。唯一能用照片先驗證的就只有置中。如果你量出來發現這張 9 分卡置中超悲劇，那不管它的殼多麼慘不忍睹，都不建議拿去重新評級，換新殼保平安才是上策。',
      regradeGuideLink: '必讀攻略：圖解何時該重新評級？何時該換新殼？',
      ...
      guideLinks: [
        ...
        {
          label: '抉擇時刻：重新評級(Regrade) 還是 換新殼(Reholder)？',
