...
export default function RetailPartners() {
  ...
  return (
    <section
      ref={sectionRef}
      className="py-28 bg-[#1e1e2e] relative overflow-hidden"
    >
      {/* Ambient gold wash */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_60%_50%_at_50%_80%,rgba(212,137,154,0.05),transparent)] pointer-events-none" />
      ...
      <div className="container-custom relative">
          ...
          {sortedPartners.map((partner, index) => {
            ...
            return (
              <a
                ...
                className="group bg-[#111116] p-10 relative overflow-hidden flex flex-col hover:shadow-[0_0_0_2px_#D4899A] transition-all duration-500"
                style={{
                  opacity: visible ? 1 : 0,
              ...
              >
                ...
                <span className="absolute -top-6 -right-2 text-[7rem] font-bold text-white/[0.025] select-none leading-none group-hover:text-[#D4899A]/[0.06] transition-colors duration-500">
                  0{index + 1}
                </span>

                {/* Top row — logo + type badge */}
                <div className="flex items-start justify-between mb-8 relative">
                  <div className="w-16 h-16 rounded-2xl border border-white/[0.08] bg-white/[0.04] flex items-center justify-center overflow-hidden group-hover:border-[#D4899A]/30 transition-colors duration-500">
                    {partner.logo ? (
                      <Image
                  ...
                  {/* Type pill */}
                  <div className="flex items-center gap-1.5 border border-white/[0.1] rounded-full px-3 py-1.5 group-hover:border-[#D4899A]/40 transition-colors duration-500">
                    {isOnline ? (
                      <Globe className="w-3 h-3 text-[#D4899A]" />
                    ) : (
                      <Store className="w-3 h-3 text-[#D4899A]" />
                    )}
                ...
                {/* Name */}
                <h3 className="text-2xl font-bold text-white mb-2 group-hover:text-[#D4899A] transition-colors duration-300 relative">
                  {'name' in i18nPartner
                ...
                {'location' in i18nPartner && (i18nPartner as { location: string }).location && (
                  <div className="flex items-start gap-2 mt-1 mb-2 relative">
                    <MapPin className="w-3.5 h-3.5 text-[#D4899A] flex-shrink-0 mt-0.5" />
                ...
                <div className="mt-auto pt-8 flex items-center justify-between relative">
                  <div className="flex items-center gap-2 text-[#D4899A] font-semibold text-sm group-hover:gap-3 transition-all duration-300">
                    <span>{label}</span>
                  ...
                  <div className="flex gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-white/10 group-hover:bg-[#D4899A]/40 transition-colors duration-300" />
                    <span className="w-1.5 h-1.5 rounded-full bg-white/10 group-hover:bg-[#D4899A]/40 transition-colors duration-500" />
                    <span className="w-1.5 h-1.5 rounded-full bg-white/10 group-hover:bg-[#D4899A]/40 transition-colors duration-700" />
                  </div>
                </div>
        ...
        >
          <div className="w-6 h-px bg-[#D4899A]/40" />
