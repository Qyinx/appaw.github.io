...
import { Search, SlidersHorizontal, ArrowUpDown, ChevronDown, X, MessageCircle, ArrowLeft, Package, Eye, ExternalLink, Hash, Globe, Tag, ZoomIn, Layers, Loader2 } from 'lucide-react';
...
function getGradeColor(grade: number, isBlackLabel?: boolean) {
...
function getCompanyStyle(company: GradingCompany) {
  switch (company) {
...
function formatGrade(grade: number, isBlackLabel?: boolean) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-8" onClick={onClose}>
      ...
      >
        ...
        <button onClick={onClose}
          className="absolute top-4 right-4 z-10 w-9 h-9 flex items-center justify-center rounded-full bg-white/[0.06] hover:bg-white/[0.12] text-white/50 hover:text-white transition-all">
          <X className="w-4 h-4" />
        </button>
        ...
        <div className="grid md:grid-cols-2">
          ...
          <div className="relative bg-gradient-to-br from-white/[0.04] to-transparent p-4 md:p-6 flex flex-col items-center justify-center min-h-[450px] md:min-h-[600px]">
            ...
            <div
              ref={imgContainerRef}
              className="relative w-full aspect-[3/4] max-w-[480px] cursor-crosshair"
            ...
            {/* Zoom hint */}
            <div className="flex items-center gap-1.5 mt-3 text-white/20 text-[10px] select-none">
...
export default function CardTradingPage() {
  ...
  // Data from API
  const { cards: allCards, loading, error } = useCards();
    ...
    <div className="flex flex-col min-h-screen bg-[#09090f]">
      ...
      <section className="relative pt-28 pb-16 overflow-hidden">
        ...
        <div className="relative container-custom z-10">
          <Link href="/business" className="inline-flex items-center gap-2 text-white/40 hover:text-[#d4a843] text-sm transition-colors mb-10 group">
            <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" />
            <span>{t.business.title}</span>
          </Link>

          <div className="max-w-3xl transition-all duration-1000" style={{ opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0)' : 'translateY(28px)' }}>
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold font-display text-white leading-[1.08] tracking-tight mb-5">{mp.title}</h1>
            ...
            <p className="text-[#9ca3af] text-lg leading-relaxed max-w-2xl">{mp.subtitle}</p>
          </div>
        </div>
      </section>
